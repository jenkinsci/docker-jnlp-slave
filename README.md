...
* `JENKINS_SECRET`: agent secret, if not set as an argument (mutually exclusive to `JENKINS_USERNAME` and `JENKINS_API_TOKEN`)
* `JENKINS_USERNAME`: username to authenticate to master (used to dynamically get the secret from master)
* `JENKINS_API_TOKEN`: password or API token to authenticate to master (used to dynamically get the secret from master)
